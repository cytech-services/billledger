...
import {
  backfillWindowEnd as baseBackfillWindowEnd,
  backfillWindowStart as baseBackfillWindowStart,
  estimatedTaxDatesForYear,
  formatDate,
  futureHorizonEnd as baseFutureHorizonEnd,
  isoDate,
  monthDateRange,
  shiftDate,
  startOfDay
} from './lib/date';
export { monthDateRange } from './lib/date';
...
function futureHorizonEnd(base = startOfDay()) {
  return baseFutureHorizonEnd(base, OCCURRENCE_FUTURE_YEARS);
}

function backfillWindowStart(base = startOfDay()) {
  return baseBackfillWindowStart(base, 2);
}

function backfillWindowEnd(base = startOfDay()) {
  return baseBackfillWindowEnd(base, 2);
}
